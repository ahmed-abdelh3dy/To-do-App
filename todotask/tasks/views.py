...
class ToTaskView(
    ...
    def get_queryset(self):
        return ToTasks.objects.filter(
            list_id=self.kwargs.get("list_pk"), status="noncompleted"
        )

    def perform_create(self, serializer):
        the_list = get_object_or_404(
            ToList, id=self.kwargs.get("list_pk"), user_id=self.request.user.id
        )
...
class SearchTaskView(mixins.ListModelMixin, generics.GenericAPIView):
    ...
    def get_queryset(self):
        return ToTasks.objects.filter(
            name=self.request.data.get("name"), list_id=self.kwargs.get("list_pk")
        )
